...
| Lepton mass spectrum (μ, τ) | 🚧 [not yet tested]<br>The McKay-distance ladder is a candidate mechanism for the hierarchy origin M5 leaves open (M8.6, GATED pending M5.21.11: named target found mass-derived, not active). Both leptons are adjudicated hits in-platform (M8.3: μ 1.03, τ 2.74, within ×3), but the pre-registered null on the corrected table gives p_A = 0.690, capping the ×3 count as uninformative about the torsion map<br>[`m8_3_method_note.md`](openwave/xperiments/m8_mit/research/findings/m8_3_method_note.md) |
