...
| Lorentz covariance (+ Coulomb = EM) | 🚧 [not yet tested]<br>Not applicable yet: no field dynamics to be covariant; supplying a field equation on S³/2I was the M8.4 program, CLOSED UNRESOLVED 2026-08-26 with no target configuration executed and no nontrivial sector spent; a target-free successor chassis program is live: M8.5-C's single attempt terminated without adjudication on a protocol defect, and the derived successor protocol M8.5-C2 is filed and frozen with its qualification attempt pending<br>[`m8_background.md`](openwave/xperiments/m8_mit/research/m8_background.md) |
