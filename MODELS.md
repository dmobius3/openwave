...
| Baryons: bound state (p, n) | ⚠️ [partially validated]<br>Protected baryon-analog states MEASURED on the certified 3D stack (the M5.22 census, adversarially audited): proton-analog = vortex column + equatorial ring, \|Q\| = 1, stationary and perturbation-stable; neutron-analog = column + two rings, Q = 0, scale-stable. Toy parameters; three-quark statistics and quantization open (lab anchor: Nature Physics s41567-025-03107-0)<br>[`m5_22_note.md`](openwave/xperiments/m5_liquid_crystal/research/findings/m5_22_note.md) |
...
| Deuteron (binding + quadrupole) | 🚧 [not yet tested]<br>Deuteron binding (m_d < m_p + m_n) and the electric quadrupole moment are the M5.22.1 subtask plus pre-registered read (iv) of the census; enters once the census produces composite candidates<br>[`m5_22_1_task_details.md`](openwave/xperiments/m5_liquid_crystal/research/tasks/m5_22_1_task_details.md) |
...
| Weak force: beta decay (n → p) | ❌ [honest negative at the 3×3 truncation]<br>The census neutron-analog (bound ring-antiring pair, exact ±1 ring charges) survives EVERY decay probe: 20 kick runs, three kick families (core-random to 23×, core-twist, ring-localized to 53× the state energy), all returning with charges intact; no n → p + e + ν channel at toy parameters. Missing angular momenta = the live suspect; the parity-violating structure stays open<br>[`m5_22_2_note.md`](openwave/xperiments/m5_liquid_crystal/research/findings/m5_22_2_note.md)<br>→ open route: the ω-twist probe [`m5_22_4_task_details.md`](openwave/xperiments/m5_liquid_crystal/research/tasks/m5_22_4_task_details.md) |
...
| Lepton mass spectrum (μ, τ) | 🚧 [not yet tested]<br>The McKay-distance ladder is a candidate mechanism for the hierarchy origin M5 leaves open (the M8.6 cross-check is GATED on M5.21.11 supplying an independent, non-circular M5-side hierarchy). Both leptons are adjudicated hits in-platform (M8.3: μ 1.03, τ 2.74, within ×3), but the pre-registered null on the corrected table gives p_A = 0.690, capping the ×3 count as uninformative about the torsion map<br>[`m8_3_method_note.md`](openwave/xperiments/m8_mit/research/findings/m8_3_method_note.md) |
