...
For what onboarding a model to OpenWave looks like in practice: M8-MIT is the live worked example, one week in. <<another_model_author>>> (model author) applied with a single discussion post and the column was scaffolded for him. The first thing that happened was a certification gate: two blind agents independently re-derived the headline eigenvalue theorem of his bedrock paper to 10-digit precision without being shown the printed constants. Since then he contributes through the normal fork-and-PR flow (two pull requests merged in his first week). One cell of his column is earned so far and the rest still reads "not yet tested"; that honesty is exactly what makes the earned cell worth something.
