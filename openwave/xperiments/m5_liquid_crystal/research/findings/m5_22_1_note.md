...
## 8. Post-close author correction (2026-08-02): the density/moments instrument reads MAGNETIC charge

The author's checkpoint reply ([`m5_22_convo.md § 2026-08-02`](../tasks/m5_22_convo.md)) rereads this note's charge-density instrument: ρ = div B/4π "seems magnetic charge"; the ELECTRIC read needs div E, with E = the curvature of the long axis, or the full form of arXiv 2108.07896. Consequences for this note's claims: the topological degree reads (\|Q\|, the exact ±1 ring degrees, the charge classes) are UNAFFECTED (author-confirmed 2026-07-30); but the § 3 dipole p_z and every § 5b-5c quadrupole value (Q2_zz −21.8 / −85.9 / −61.5) are magnetic-sector moments, so **the "electric quadrupole sign tension vs the physical deuteron" is UNSETTLED until the div E instrument recomputes it** (staged as the [M5.22.2](../tasks/m5_22_2_task_details.md) opening move, both E variants). Sections above retain their original wording as the run record; read "electric" in §§ 3, 5b, 5c through this correction.

**RESOLVED BY MEASUREMENT (2026-08-05, [M5.22.2 note § 2-3](m5_22_2_note.md)):** both named variants were built and calibrated. Only the paper's dual-curvature form passes the Gauss-law calibration (analytic hedgehog → Coulomb r̂/r² at 0.7% median error, quantized ±1 on the known charged states); the literal field-line curvature does not quantize on any control. The dual-curvature form is numerically the SAME array this note called B, so under the paper's dual mapping (space-space curvature = electric sector) the moment values above stand as electric-sector reads, and the candidate's NEGATIVE quadrupole sign SURVIVES (−21.8 at n = 32, −61.5 at n = 48): the sign tension is real under the calibrated instrument, with the dual mapping itself author-gated at the next checkpoint.
