...
> **Status: FROZEN (2026-08-07, user call).** This document is the pre-registration artifact required by the amended M8.6 reopening condition 3, route (b) ([readiness note § 8](../../../m8_mit/research/findings/m8_6_readiness_note.md)). It contains the derived asymptotic forms (§ 1), the frozen ladder protocol (§§ 2-6), and the compliance audits (§§ 7-9). **No rung has been measured, no fit run, no ratio computed**: the ladder compute is a separate follow-on run against this frozen document. Freeze record: the author sanity-check of § 1 was sent 2026-08-06 and answered 2026-08-07 (public reply, captured in [`m5_21_convo.md`](../tasks/m5_21_convo.md)); the reply endorsed the practical-approximation route and raised no objection to any derived form, while leaving the three specific questions unanswered; since § 1 covers every branch of them (θ unconstrained in the fit, the gain exponent a measured q ∈ [2, 4] window, the anchors order-of-magnitude with stated insensitivity), the user called the freeze. Pinning commit SHA: `PENDING-FREEZE-COMMIT` (filled in by the sole permitted post-freeze edit, which records the SHA of the commit that introduced this FROZEN header). Any other edit after this point voids route (b) (failure is terminal, no second framework).
