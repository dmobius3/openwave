# M5.22.9: nuclei at scale (the far arc): larger nuclei, halo nuclei, alpha clusters, toward the table of nuclides

**Status**: 🚧 PLANNED STUB, far-gated. **Renumbered from M5.22.1 on 2026-07-30** (series re-plan, [M5.22 § The nuclear-hunt series plan](m5_22_task_details.md): the toy successor subtasks took M5.22.1-.3 so numeric order = run order; records written before 2026-07-30 call this task "M5.22.1"). **Split out of [M5.22](m5_22_task_details.md) on 2026-07-27** (user decision): M5.22 keeps everything that runs on the hardware this project actually has (the toy 3×3 vortex-knot census), and this task carries everything that does not. The split exists so the resource question is stated honestly instead of buried inside a single task that is half-runnable: what is promised to the author is the toy ladder, and what needs compute we do not have is named here and priced on the [README § Resource Contributors](../../../../../README.md#resource-contributors) page. Records written before the split call this scope "M5.22 Phase 2". **Sequencing CONFIRMED by the author 2026-07-27** ([`m5_22_convo.md § 2026-07-27 afternoon`](m5_22_convo.md)): focus on the small nuclei first; the larger ones need separately prepared large simulations, to be thought about after the small ones come out right.

## Scope: the rungs that do not fit on current hardware

The ladder is the author's ([`m5_22_convo.md § 2026-07-22`](m5_22_convo.md), extended 2026-07-26); the toy series ([M5.22](m5_22_task_details.md) → [M5.22.3](m5_22_3_task_details.md)) runs its lower rungs, this task owns the upper ones (plus any toy rung the feasibility gate folds in here).
...
| Gate | Content |
| --- | --- |
| The toy results | The toy series ([M5.22](m5_22_task_details.md) → [M5.22.3](m5_22_3_task_details.md)) has to land first: the lower rungs are the instrument validation for these |
