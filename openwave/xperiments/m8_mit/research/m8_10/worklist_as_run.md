...
**2.** For a general, unnormalized `v ∈ V_3`, the fibre vector `w(v)` with `Π_6 N(Φ_{σ,v}) = Φ_{σ,w(v)}`, in closed form, in both sectors. Express it through the `M_K` above, with every coefficient exact; if you work in another basis, give its exact relation to the `M_K`.
...
**4.** At `R1` to `R5`: solve the order-`a³` equation for `ξ`, and report `‖Π_n ξ‖²/g²` at every level `n` of the sector, exactly, including any level where it vanishes. State your method, and for rational identification the precision and the denominator bound.
...
**7.** At `R1` to `R5` and in each sector, `λ₄/g²` exactly, with the method; and the sign of `λ₄` for `g ≠ 0`, with an argument rather than only the values.

**8.** At `R1` to `R5`, the ratio of `λ₄` in the 3-dimensional sector to `λ₄` in the 4-dimensional sector, exactly. Then say whether one number `c` gives `λ₄(3-dimensional) = c · λ₄(4-dimensional)` at every ray, and answer the same question for item 3's multiples minus 1.

**9.** At `R1` to `R5`, check the order-`a³` equation for `ξ` as a vector identity: build the `SU(2)` Casimir from explicit spin-matrix generators, apply it to the `ξ` you computed to obtain `−Δξ`, and report the numerical residual. Substituting the formula `n(n+2)` does not count as the check.
