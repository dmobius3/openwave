...
**Implementer eligibility, and it is permanent.** Any context that has been exposed to the
target-source records, or to the answer packet or to its construction or review material, is
PERMANENTLY ineligible to serve as the implementer context. No teardown, summary, restatement
or fresh prompt restores eligibility, because what disqualifies a context is what it has been
shown and not what it still holds. The implementer is deliberately unassigned, which is exactly
the condition under which this is easiest to forget.
...
| `convention_map` | structured, FOUR items: the § 5.4 bridge; the § 5.4 orientation-anchor rule; the § 4.2 basing reference; and, as a SEPARATE item from that reference, the § 4.2 `basing.evaluation` declaration reproduced verbatim. The last two are distinct and neither substitutes for the other: the reference names § 4.2, the declaration carries the evaluation convention § 4.2 states. "The evaluation convention" here means `basing.evaluation` and nothing else; no other encoding may be filled in against it |
...
| `H_*(C_*) ≅ (Z, 0, 0, Z)` for `C_*` read as a complex of free `Z`-modules, that is the universal cover `S³`. Established INTEGRALLY AND IN EVERY DEGREE: `im ∂ₙ₊₁ = ker ∂ₙ` as lattices at each step, from containment (`∂ₙ₊₁∂ₙ = 0`), the exact ranks, and an exact SATURATION certificate for the image of EVERY nonzero boundary map, that is for `im ∂₁`, `im ∂₂` and `im ∂₃`. A certificate for `im ∂₃` alone establishes degree 2 only and leaves degrees 0 and 1 resting on the ranks, which are the RATIONAL statement this row warns is blind to finite index. Admission rule, unchanged and applying to each certificate separately, stated as a property rather than a list: the certificate must determine the elementary divisors exactly, by integer arithmetic, with no prime and no sample on the accept side. Examples under that rule: a maximal minor of determinant `±1`, Smith or Hermite data, or a unimodular-elimination certificate. Note the asymmetry: a determinant route always returns a number, while an elimination route may stall, and a stalled elimination is INCONCLUSIVE rather than a pass. A mod-`p` battery is a reject screen and is NOT sufficient on the accept side, since no finite prime set excludes an unseen index prime | that `C_*` is the chain complex of `S³` and not merely a complex with the right RATIONAL invariants. A complex can pass every other gate in this table while `im ∂₃` sits at finite index inside `ker ∂₂`: `∂₃ → k·∂₃` leaves `∂∂ = 0`, `ε(∂₃) = 0`, the augmented homology and per-irrep acyclicity all untouched, because each is a rank statement or is computed from `ε(∂₂)` alone, while multiplying every reproduced torsion by `k` to a power. A SEPARATE failure mode, which no scalar mutation produces and which is why the certificate is scoped to every degree rather than to `im ∂₃`: multiply ALL of `∂₂` by the central non-unit `6 − 5z`, `z` the central involution. Centrality preserves every chain relation, so `∂₃∂₂ = 0` and `∂₂∂₁ = 0` both survive; `ε(6 − 5z) = 1`, so `ε(∂₂)` and everything computed through the augmentation is unmoved; and `6 − 5z` acts invertibly on every irrep, as `1` where `z ↦ 1` and as `11` where `z ↦ −1`, so the per-irrep acyclicity, the exact ranks and a certificate for `im ∂₃` all still pass. The damage lands in degree 1, which those gates never inspect. Modulo 11 the faithful part of the source dies and the rank of `∂₂` falls by exactly the dimension those irreps carry, so `im ∂₂` is no longer saturated in `ker ∂₁` and `H₁(C_*)` acquires 11-torsion: the complex is not `S³`'s and every reproduced torsion is wrong by that factor. No finite prime list closes this, which is the same reason given above for `∂₃ → k·∂₃` one degree up. Augmentation 1 is what carries it past gates a scalar cannot pass, since an integer scalar on `∂₂` moves `ε(∂₂)` and the augmented-homology row catches it. This gate is the only one in the table that sees the integral lattice |
