...
def references(s_val):
    ...
    # 50-dps eigenvalues: mp.eigsy on the FULL matrix is feasible at 116 via symmetric split?
    # cheaper: eigsy per nu-block. Blocks: indices grouped by (nu, re/im part) pair coupling.
    # J couples (i,j) only when nu_i = nu_j (both Jc and Ju phase rules force it, and the
    # diagonal is nu-preserving), in BOTH the re and im sectors, so block = {i: nu_i = v} for
    # each v, separately in the re sector and the im sector.
    mus_mp = []
    vecs = np.zeros((2 * n, d_c))
    got = 0
    for sector in (0, 1):
        for v in sorted({e[4] for e in BASIS}):
            ids = [i for i, e in enumerate(BASIS) if e[4] == v]
            off = sector * n
    ...
    for cidx, (val, ids, q) in enumerate(cluster):
...
armN = all(float(mp.mpf(results[k]["splitting"])) > 1e-8 and results[k]["leakage"] > 1e-12
           for k in results)
armS = all(results[k]["sep"] >= 10 for k in results)
armX = x_err < 1e-10
...
ok = armP and armX and armM and armN and armS
