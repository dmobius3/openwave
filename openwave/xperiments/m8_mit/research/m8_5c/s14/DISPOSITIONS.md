...
## Hash citations in this file

This table was written at the ORIGINAL one-pass run. Items 4, 5 and 6 were superseded twice
after it (the § 14 supplement records both rounds and why), so any hash quoted in the rows
above is the value at that first run and is NOT the shipped value. The shipped hashes live
in exactly two places, both regenerated at filing: `raw/package_hashes.txt`, which covers
every tracked member of this package, and the protocol's § 15 pin table.
