...
| --- | --- | --- | --- |
| OQ1 | Does ANY reasonable nonlinear field equation on S³/2I have defect or standing-wave solutions whose energies realize the McKay slot structure? | M8.4, CLOSED UNRESOLVED 2026-08-26 | 🚧 open, and NEVER TESTED on the twisted branch: M8.4 closed on two instrument failures with no dynamics run and no sector spent ([closeout](findings/m8_4_closeout.md)), so OQ1's dynamical branch awaits the target-free M8.5-C chassis. The NATIVE branch closed by theorem 2026-08-18 ([kinematic close](findings/m8_4_kinematic_close.md)): single-valued quotient fields carry no nontrivial slot at any level, so the live branch runs on M8-owned twisted objects, `M4_int` first |
