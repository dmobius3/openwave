...
| First correction at the critical rays | the range equation carried one order at the four symmetry-pinned rays of the channel-selection paper: the first state correction and `λ₄ = −3g² Σ_n ‖Π_n N(Φ)‖² / (n(n+2) − 48)`, negative at all eight expansions; the fixed-ray expansion fails at the pentagonal pyramid | platform task M8.10, author-proposed ([#546](https://github.com/openwave-labs/openwave/pull/546)) | ✅ VERIFIED IN-PLATFORM (M8.10, 2026-09-13): blind, two agents plus an adversarial audit, every value exact. Formal coefficients through order `a⁵` only ([`findings/m8_10_method_note.md`](findings/m8_10_method_note.md)) |
| Local branch germs and the tilt | the formal expansions at six level-6 critical rays are Taylor expansions of local branch germs for sufficiently small amplitude (equivariant Lyapunov-Schmidt with a slice, trivial branch divided out first); at the pyramid and the prism the direction tilts at order `a²`, `L_T v = −Π_T DN_Φ[ξ]` with `L_T = (w₆/4) Hess_T r̂₆`, and the tilt leaves `λ₄` unchanged | platform task M8.11, author-proposed ([#554](https://github.com/openwave-labs/openwave/pull/554)) | ✅ VALUES VERIFIED IN-PLATFORM, 🔶 EXISTENCE AN AUDITED ARGUMENT (M8.11, 2026-09-18): blind, two agents, every value exact; the proof graded step by step by one AI auditor. No radius or stability ([`findings/m8_11_method_note.md`](findings/m8_11_method_note.md)) |
| The reduced Morse census | of the character-fixed loci of `ℙ(V₃)` with projective dimension at most one, six points and seven lines, the complete critical set carries exactly ten orbits; at each, the signature of `M_u = Hess N(u) − 4N(u)·I` on `N_u = T_u ⊖ O_u` gives the reduced Morse index for both signs of `g`, `n₋` for `g > 0` and `n₊` for `g < 0`; `v₁` is the only degenerate orbit, with kernel the line direction `span{v₋₃, i·v₋₃}`; the coherent orbit is the unique minimum at `1/924` and the hexagon the maximum at `463/924` | platform task M8.12, author-proposed ([#571](https://github.com/openwave-labs/openwave/pull/571)) | ✅ VALUES AND ARGUMENTS VERIFIED IN-PLATFORM (M8.12, 2026-09-21): blind, two agents offline, every value exact and reproduced on an independent maintainer route; completeness on the loci by elimination, graded SOUND in both rooms. ⚠️ No stability claim, and three orbits with `C₂`, three-dimensional `C₃` or trivial stabilizer lie outside the census ([`findings/m8_12_method_note.md`](findings/m8_12_method_note.md)) |
