...
**Note, 2026-09-21, after the run.** Item 5b asks for the second variation along the tangent directions to a class at an interior orbit, normalized in `Re⟨·,·⟩` once each has been tested for a vanishing projection onto `N_u`, but it does not say whether to normalize that projection or the original tangent. P4's pass condition settles it, since it takes the tangents orthogonal to the orbit directions. The run showed the gap: the two rooms reported different numbers for the same orbit, the D2 ray, each correct under the normalization it stated and related to the other by exact projection factors. The D2 ray is not among P4's orbits, so the divergence fell where nothing was scored, and P4 reproduced under its own convention ([method note](../findings/m8_12_method_note.md), § 5.3). A rerun would normalize each direction's projection onto `N_u`. No frozen value moves, the worklist stays as run, and this is the second unpinned convention the M8.12 record has found in a scored quantity, after N1's residuals in [#571](https://github.com/openwave-labs/openwave/pull/571).
