...
| P2 | On the D₃ locus, the chart restriction is `(100\|z\|⁴ − 20x² + 148y² + 463)/(231(\|z\|² + 2)²)` | paper § 5.8 | reproduced exactly | differs |
...
| ID | Claim | Standing | Pass condition | Fail condition |
|---|---|---|---|---|
| N1 | At the non-critical point the worklist supplies, `u = (v₃ + v₁ − v₋₂)/√3`, where `924·r̂₆ = 204`, the tangential gradient of `r̂₆` is `2√1002/297`, and the orbit directions are not annihilated by `Hess N − 4N·I`, the largest residual being `2√(312√15 + 4170)/297` | new | all three exact quantities reproduce, and the gradient and the residual are nonzero | any of the three differs, or either the gradient or the residual vanishes, which would mean the instrument cannot fail. Both vanish exactly at a critical point, which `s1_control.py` checks as its arm |
