...
1. **`METHOD_AND_GATE_MANIFEST.md`**: the route you selected within the protocol's § 6 class,
   ...
   treats a manifest written after the fact as a different and weaker object.

   **The manifest is not complete until every manifest-defined construction or convention
   that can be checked from the permitted inputs has undergone its own target-blind
   internal-consistency check, with those checks recorded in the manifest. Defects found
   during this pre-implementation validation may be corrected and recorded before
   implementation begins.**

   **The manifest carries a coverage table with one row for every construction and
   convention the manifest itself defines. Each row names either the validation artifact
   and the specific check that tests it, or states explicitly that it is not checkable from
   the permitted inputs before implementation and why. The `MANIFEST STATUS: FINAL` line may
