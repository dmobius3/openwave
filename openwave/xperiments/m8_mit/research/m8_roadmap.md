...
>
> **Blind vs independent-method verification (M8.2 close-out thread, 2026-07-28).** Blind
> means the verifying agent has not seen the claimed values
> ([`ONBOARDING_MODELS.md § 3.2`](../../../../ONBOARDING_MODELS.md#32-the-maintainer-sequence)
> step 5). A context that has already seen the targets can still verify by an independent
> method, but the result is reported as **independent-method reproduction**, never as blind;
> the ceiling on the claim is set by what the verifying context has seen, not by how separate
> the implementation is. M8.1 was blind (targets withheld from both agents) and M8.1.1 is
> blind by construction. M8.5's table reproduction cannot be: the context that produced M8.2
> holds the fixtures, and the maintainer reproduced the tables during the
> [PR #350](https://github.com/openwave-labs/openwave/pull/350) review. The M8.2 lock's § 3
> requirement (independently implemented, may compare against the script but not call it,
> import its tables, or share its derived fixtures) is IMPLEMENTATION independence and stands
> unchanged; the author's added firewall extends it to the CONTEXT (implement with no M8.2
> internals loaded), which is the part that AI-run work can silently violate.
>
