...
| Cross-model payoff regardless | M8.6 (the McKay rule vs M5's lepton hierarchy) is the ONE candidate mechanism on the table for M5's open hierarchy origin, and pays off whichever way it lands. GATED 2026-07-29: `1 : 5.9 : 15.1` is `Λ := m^(1/3)` from the masses, not measured M5 output, so the comparison waits on the M5.21.11 physical-parameter bridge ([readiness note](findings/m8_6_readiness_note.md)) |
