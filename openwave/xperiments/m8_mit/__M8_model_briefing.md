...
| M8.10 | ✅ ALL FROZEN CLAIMS REPRODUCE (2026-09-13): the fourth bedrock paper's range equation carried one order at its four critical rays in sectors `3′` and `4`, blind, two agents and an adversarial audit. The first state correction and `λ₄` are exact, `λ₄ < 0` at all eight expansions, and the negative control fired at the pentagonal pyramid. Formal coefficients only: no branch, existence or stability ([`research/findings/m8_10_method_note.md`](research/findings/m8_10_method_note.md)) |
| M8.11 | ✅ ALL FROZEN VALUES REPRODUCE (2026-09-18): at the four symmetry-pinned rays and at the pentagonal pyramid and trigonal prism, in sectors `3′` and `4`, the formal expansions are Taylor expansions of local branch germs, for sufficiently small amplitude. The existence theorem stands as an **audited argument**, not a verified theorem; the order-`a²` tilt at the two new rays is exact, derived blind by two agents. No radius, stability or finite-amplitude claim ([`research/findings/m8_11_method_note.md`](research/findings/m8_11_method_note.md)) |
| M8.12 | ✅ EVERY FROZEN CLAIM REPRODUCES, `G2`'s uniqueness clause unresolved (2026-09-21): the reduced Morse census of the level-6 quartic `r̂₆`, ten critical orbits with their exact values, transverse dimensions, signatures and characteristic polynomials, the `v₁` kernel and both ends, blind, two agents and an adversarial audit that found no mathematical defect. Sector-independent by D7. **No stability claim**: this is the Morse index of the leading reduced quartic transverse to the phase-and-rotation orbit, not the stability of a branch germ. No MODELS.md cell moves and M8.7's gate is unchanged ([`research/findings/m8_12_method_note.md`](research/findings/m8_12_method_note.md)) |
