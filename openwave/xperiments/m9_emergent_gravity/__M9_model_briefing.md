...
| Gravity: Newton limit (GEM) | ⚠️ inherited, C1 PASS / C2 FAIL. Attractive \(GM/r^2\) to \(\le 1.3\%\) on a Dirichlet cube; isolated \(\Phi=-GM/r\) fails (\(26\)--\(35\%\)) because the locked box is not isolated space. M9.38 sources the same C1 from an entanglement \(M_{\mathrm{hat}}\) (\(n=65\) residuals \(\le 3.0\%\)); Poisson is still inherited, not derived. Not GEM, not FGHMV, no `MODELS.md` column. [`research/findings/m9_2_newton_note.md`](research/findings/m9_2_newton_note.md), [`research/findings/m9_38_from_kappa_note.md`](research/findings/m9_38_from_kappa_note.md) |
