...
| Honest residuals | Multi-digit \(I_B\) documented negative; FGHMV copy onto the cosmological horizon obstructed; the spinless vacuum of Einstein+\(\Lambda>0\) \emph{is} de Sitter (metric, not a holographic selection); nonlinear EC closed as an axial obstruction; UV pair not selected; an \(S^3\)/Bloch picture is a guess; as a description of \(K\) it fails (Paper 32); point-source \(\delta S\) vs site-energy is not a CHM first law even in 1d (Paper 36); no DM particle; masses and generations unexplained |
...
| A2 local-\(X\) modular ansatz | ⚠️ local hop is CHM-type (Paper 25). Horizon first law: a radial weight beats flat (Papers 27, 29); the parabola is not selected over \(R-r\) (Paper 29, tie). Tracking floors fail. A 3d point source selects enclosed energy, not CHM (Paper 35); the same instrument fails the 1d CHM theorem case (Paper 36), so Paper 35 is not a 3d no-go. [`research/findings/m9_27_1d_point_chm_note.md`](research/findings/m9_27_1d_point_chm_note.md) |
