...
| Honest residuals | Multi-digit \(I_B\) documented negative; FGHMV copy onto the cosmological horizon obstructed; the spinless vacuum of Einstein+\(\Lambda>0\) \emph{is} de Sitter (metric, not a holographic selection); nonlinear EC closed as an axial obstruction; UV pair not selected; no DM particle; masses and generations unexplained |
...
| A2 local-\(X\) modular ansatz | ⚠️ not refuted on 1d/2d/3d lattice Dirac or on a \(3+1\)D diamond waist. The local hop tracks a CHM-type envelope (Paper 25). The first law tracks a *local* kernel; the envelope is not selected (Paper 26, C3 FAIL, auditor CONFIRMED). [`research/findings/m9_16_first_law_note.md`](research/findings/m9_16_first_law_note.md) |
