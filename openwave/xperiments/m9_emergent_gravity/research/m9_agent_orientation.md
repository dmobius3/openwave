...
| Precision then efficiency | **Priority 1 is precision. Priority 2 is efficiency.** Physics, logic, and math first. Prefer exact bases and multiprecision (`mpmath`) over float64 LAPACK when the object admits it. Do not thin a pre-register, drop digits, or switch to a noisier eigensolve to save wall time. After the result is exact enough to trust, compile the same helpers with Nuitka (`scripts/compile_nuitka.sh`) so mpmath loops run as C. Do not replace mpmath with float64 for speed |
