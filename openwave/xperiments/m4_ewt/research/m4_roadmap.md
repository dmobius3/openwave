...
| [M4.12](tasks/m4_12_task_details.md) | Newton force with fully geometric G | Update M4.10 to use the self-consistent trinity `G_geom`, `hbar_geom`, `lambda_l` from the updated M4.7 chain. Replace the normalization-consistency path with an explicit amplitude derivation. Add an independent audit script. [PR #553](https://github.com/openwave-labs/openwave/pull/553) wrote the amplitude in chain factors, `2 G_geom M / c^2` by value, so that clause stays open. Contributed by Łukasz Smoliński | [M4.7](tasks/m4_7_task_details.md), [M4.10](tasks/m4_10_task_details.md) |
...
| [M4.11](tasks/m4_11_task_details.md) | N as the internal lock-in point: drift, resolution, grouping | Scans `N` from 500 to 1100 and measures each sector's resolution, its relative error over its log-derivative. Only `G` (4.0e-5, exponent -12 through the derived `lambda_l`) and `a_tau` (1.4e-3) resolve `N`; their separately-optimal values agree to 0.14%, while `alpha` gives no constraint (635%). The `a_tau` target is the SM prediction, not a measurement. Contributed by Łukasz Smoliński, [PR #531](https://github.com/openwave-labs/openwave/pull/531) | 2026-09-08 |
